...
pairs <- read.table(paste("~/projects/the_1001/selection/fst/2/results/matches_relicts_nonrelicts_paired.txt", sep=""), header=T, sep="\t", as.is=T, stringsAsFactors=F);
...
for( i in 1:length(uniquePopulations)){
	...
	for( j in 1:5 ){
		...
		outputDirectory <- paste(callMethodId, "/imp", snpImputationThreshold, "/", popName, "/chr", j, "/slice1", sep="");
		...
		outputFileName <- paste(callMethodId, "/imp", snpImputationThreshold,  "/", popName, "/", "chr", j, "/slice1/", popName, "_chr", j, ".n_", nrow(chr_j), ".slice1.omega.txt", sep="");
