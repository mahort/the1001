...
subpops <- unique(metadata[,"Admixed"]);
subpops <- subpops[which(!is.na(subpops))];
...
for( j in 1:5 ){
	...
	outputDirectory <- paste(callMethodId, "/imp", imputationThreshold, "/global/chr", j, "/slice1", sep="");
	...
	outputFileName <- paste(callMethodId, "/imp", imputationThreshold,  "/global/", "chr", j, "/slice1/global_chr", j, ".n_", nrow(chr_j), ".slice1.omega.txt", sep="");
