...
dataset <- "admixed";
dataset <- match.arg(dataset, c("admixed", "popn"));

setwd(paste("~/projects/the_1001/selection/fst/", callMethodId, "/imp", snpImputationThreshold, "/pairwise/", dataset, "/results/", sep="")); #")
files <- list.files(path=".", pattern="combined.fst.txt$");
print(files);

summaryStatistic <- "max";
summaryStatistic <- match.arg(summaryStatistic, c("mean", "max", "median"));
